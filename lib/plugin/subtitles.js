 ...
 * Subtitles will be generated in SRT as well as WebVTT format (standard for HTML5)
 * #### Configuration
 * ```js
 * plugins: {
 *  subtitles: {
...
module.exports = function () {
  ...
  event.dispatcher.on(event.test.after, async (test) => {
    if (test && test.artifacts && test.artifacts.video) {
      ...
      // For a VTT file, every subtitle has to be in the format as mentioned below:
      // WEBVTT
      //
      // 1
      ...
      // HH:mm:ss,SSS --> HH:mm:ss,SSS
      // [title]
      for (const subType of ['srt', 'vtt']) {
        let subtitle = subType === 'vtt' ? 'WEBVTT\n\n' : '';
        const msIdentifier = subType === 'vtt' ? '.' : ',';
        const artifactName = subType === 'vtt' ? 'subtitleVTT' : 'subtitle';
...
`;
        ...
        await fsPromise.writeFile(`${artifactsDirectory}/${fileName}.${subType}`, subtitle);
        test.artifacts[artifactName] = `${artifactsDirectory}/${fileName}.${subType}`;
      }
    }
  });
};
